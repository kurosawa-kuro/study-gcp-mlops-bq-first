...
* :class:`BigQueryCandidateRetriever` — runs BigQuery VECTOR_SEARCH against
  ``feature_mart.property_embeddings`` joined to ``property_features_daily``.
...
class BigQueryCandidateRetriever:
    """Lexical candidate generation via BigQuery VECTOR_SEARCH.
    ...
    def retrieve(
        ...
        *,
        ...
        # top_k here is the number of final results; internally we keep a larger
        # candidate pool (100) so there is headroom for the rerank step once it
        # lands in Phase 6. Phase 4 returns top_k directly as lexical order.
        candidate_pool = max(top_k, 100)
        query = f"""
            WITH base AS (
              SELECT
        ...
        params = [
            ...
            bigquery.ScalarQueryParameter("pool_size", "INT64", candidate_pool),
